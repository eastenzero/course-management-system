...
class SchedulingAlgorithm:
    ...
    def find_best_slots(self, constraint: ScheduleConstraint) -> List[ScheduleSlot]:
        ...
        # 筛选可用的时间槽
        for slot in list(self.available_slots):  # 转换为列表以避免在迭代时修改集合
            # 检查基本约束
...
def main():
    ...
    if result:
        ...
        with open('scheduling_result_test_fixed.json', 'w', encoding='utf-8') as f:
            json.dump(output_data, f, ensure_ascii=False, indent=2)
        
        print(f"💾 排课结果已保存到 scheduling_result_test_fixed.json")
