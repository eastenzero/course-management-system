...
def import_schedule_final():
    ...
    try:
        ...
        print(f"准备导入 {len(final_assignments)} 条优化排课记录")
        ...
        for i, assignment in enumerate(final_assignments):
            try:
                ...
                Schedule.objects.create(
                    ...
                    status='active',
                    notes="算法生成 - 遗传算法 - 最终优化版"
                )
                
                success_count += 1
                
                if (i + 1) % 4 == 0:
                    print(f"已导入 {i + 1}/{len(final_assignments)} 条记录")
