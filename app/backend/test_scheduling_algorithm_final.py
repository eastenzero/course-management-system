...
class SchedulingAlgorithm:
    ...
    def __init__(self, semester: str, academic_year: str):
        ...
        # 记录已使用的槽位，避免重复分配
        self.used_slots: Set[ScheduleSlot] = set()
    ...
    def calculate_slot_score(self, constraint: ScheduleConstraint, slot: ScheduleSlot) -> float:
        ...
        # 偏好教室权重
        if not constraint.preferred_classrooms or slot.classroom in constraint.preferred_classrooms:
            score += 20
        
        # 偏好时间段权重
        if not constraint.preferred_time_slots or slot.time_slot in constraint.preferred_time_slots:
            score += 15
        
        # 偏好星期权重
        if not constraint.preferred_days or slot.day_of_week in constraint.preferred_days:
            score += 10
    ...
    def find_best_slots(self, constraint: ScheduleConstraint) -> List[ScheduleSlot]:
        ...
        for slot in self.available_slots:
            ...
            # 检查基本约束
            if (constraint.preferred_classrooms and slot.classroom not in constraint.preferred_classrooms):
                continue
                
            if (constraint.preferred_time_slots and slot.time_slot not in constraint.preferred_time_slots):
                continue
                
            if (constraint.preferred_days and slot.day_of_week not in constraint.preferred_days):
                continue
            ...
            # 检查冲突
            if (self.check_teacher_conflict(constraint.teacher, slot) or
                self.check_classroom_conflict(slot.classroom, slot)):
                continue
        ...
        for i, constraint in enumerate(sorted_constraints):
            try:
                ...
                if len(best_slots) >= constraint.sessions_per_week:
                    ...
                    # 尝试放宽约束条件重新分配
                    relaxed_slots = self._try_relaxed_constraints(constraint)
                    if len(relaxed_slots) >= constraint.sessions_per_week:
                        self.assigned_slots[constraint] = relaxed_slots
                        # 更新冲突跟踪
                        self._update_conflict_tracking(constraint, relaxed_slots)
                        successful_assignments += 1
                        print(f"  约束 {i+1}: {constraint.course.name} - 放宽约束后成功分配 {len(relaxed_slots)} 个时间槽")
                    else:
    ...
    def _try_relaxed_constraints(self, constraint: ScheduleConstraint) -> List[ScheduleSlot]:
        """尝试放宽约束条件重新分配"""
        print(f"    尝试放宽约束条件: {constraint.course.name}")
        
        # 备份原始偏好
        original_classrooms = constraint.preferred_classrooms.copy() if constraint.preferred_classrooms else []
        original_time_slots = constraint.preferred_time_slots.copy() if constraint.preferred_time_slots else []
        original_days = constraint.preferred_days.copy() if constraint.preferred_days else []
        ...
        try:
            ...
            if constraint.preferred_classrooms:
                ...
                best_slots = self.find_best_slots(constraint)
                if len(best_slots) >= constraint.sessions_per_week:
                    print(f"      放宽教室限制后成功分配 {len(best_slots)} 个时间槽")
                    return best_slots
            ...
            if constraint.preferred_time_slots:
                ...
                best_slots = self.find_best_slots(constraint)
                if len(best_slots) >= constraint.sessions_per_week:
                    print(f"      放宽时间段限制后成功分配 {len(best_slots)} 个时间槽")
                    return best_slots
            ...
            if constraint.preferred_days:
                ...
                best_slots = self.find_best_slots(constraint)
                if len(best_slots) >= constraint.sessions_per_week:
                    print(f"      放宽日期限制后成功分配 {len(best_slots)} 个时间槽")
                    return best_slots

        except Exception as e:
            print(f"      放宽约束条件时发生错误: {str(e)}")
        finally:
            # 恢复原始偏好
...
def create_auto_schedule(semester: str, academic_year: str) -> Dict:
    ...
    for i, course in enumerate(courses):
        ...
        # 计算每周课时数
        sessions_per_week = min(max(course.max_students // 30, 1), 4)  # 简化计算，至少1节
...
def main():
    ...
    if result:
        ...
        with open('scheduling_result_test_final.json', 'w', encoding='utf-8') as f:
            json.dump(output_data, f, ensure_ascii=False, indent=2)
        
        print(f"💾 排课结果已保存到 scheduling_result_test_final.json")
