...
class MillionDataGenerator:
    ...
    def generate_million_students(self, target_count=800000):
        ...
        for batch_start in range(0, target_count, self.batch_size):
            ...
            if batch_start % (self.batch_size * 10) == 0:
                ...
                print(f"   📊 进度: {created_count:,}/{target_count:,} ({progress:.1f}%) | "
                      f"速度: {speed:.0f} 条/秒 | 内存: {memory_mb:.0f}MB")
                
                # 内存管理
                if memory_mb > 1500:  # 超过1.5GB时执行垃圾回收
                    gc.collect()
    ...
    def generate_million_teachers(self, target_count=50000):
        ...
        for batch_start in range(0, target_count, self.batch_size):
            ...
            if batch_start % (self.batch_size * 5) == 0:
                progress = (created_count / target_count) * 100
                memory_mb = self.monitor_memory()
                print(f"   📊 教师进度: {created_count:,}/{target_count:,} ({progress:.1f}%) | 内存: {memory_mb:.0f}MB")
    ...
    def generate_million_courses(self, target_count=30000):
        ...
        # 获取教师用户用于分配
        teachers = list(User.objects.filter(user_type='teacher').values_list('id', flat=True))
        if not teachers:
        ...
        for batch_start in range(0, target_count, self.batch_size):
            ...
            if batch_start % (self.batch_size * 2) == 0:
                progress = (created_count / target_count) * 100
                memory_mb = self.monitor_memory()
                print(f"   📊 课程进度: {created_count:,}/{target_count:,} ({progress:.1f}%) | 内存: {memory_mb:.0f}MB")
        
        elapsed = time.time() - start_time
        print(f"   ✅ 课程创建完成: {created_count:,} 条，耗时 {elapsed:.1f} 秒")
        return created_count
    
    def generate_million_enrollments(self, target_count=150000):
        """生成选课记录"""
        print(f"\n📝 开始生成 {target_count:,} 条选课记录...")
        
        # 获取学生和课程ID
        students = list(User.objects.filter(user_type='student', username__startswith='million_').values_list('id', flat=True))
        ...
        for batch_start in range(0, target_count, self.batch_size):
            ...
            if batch_start % (self.batch_size * 5) == 0:
                progress = (created_count / target_count) * 100
                memory_mb = self.monitor_memory()
                print(f"   📊 选课进度: {created_count:,}/{target_count:,} ({progress:.1f}%) | 内存: {memory_mb:.0f}MB")
...
def main():
    ...
    try:
        ...
        enrollments_created = generator.generate_million_enrollments(150000)
