...
class SchedulingAlgorithm:
    ...
    def find_best_slots(self, constraint: ScheduleConstraint) -> List[ScheduleSlot]:
        ...
        for slot, score in candidate_slots:
            ...
            # 如果避免连续排课，检查是否在同一天
            if constraint.avoid_consecutive and slot.day_of_week in used_days:
                continue
                
            selected_slots.append(slot)
            used_days.add(slot.day_of_week)
    ...
    def _attempt_conflict_resolution(self, failed_assignments: List[Dict]) -> int:
        """尝试解决冲突的分配"""
        resolved_count = 0
...
def create_auto_schedule(semester: str, academic_year: str, course_ids: List[int] = None) -> Dict:
    """
    自动排课主函数
